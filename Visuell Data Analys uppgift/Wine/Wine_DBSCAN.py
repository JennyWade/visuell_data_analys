...
class DBScan:
    ...
    def dbscan(self):
        db = DBSCAN(eps = 3, min_samples = 28).fit(self.scaled_data)
    ...
    def pca(self):
        ...
        db = DBSCAN(eps = 3, min_samples = 28).fit(self.pca_data)
    ...
    def difference(self):
        ...
        ax[0].scatter(x1, y1, c=self.df['target'], cmap=plt.cm.Set1)
        ax[1].scatter(x2, y2, c=self.labels)
